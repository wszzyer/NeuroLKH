...
void CreateCandidateSet(double* invec)
{
    ...
    if (MaxCandidates > 0 &&
        ...
        ReadPenalties(0);
        if (!(CandidatesRead = ReadCandidates(MaxCandidates, 0) ||
    ...
    } else if (NeuroLKH == 0 && ((CandidatesRead = ReadCandidates(MaxCandidates, 0) ||
                ReadEdges(MaxCandidates)) || MaxCandidates == 0)) {
        AddTourCandidates();
        if (CandidateSetSymmetric)
            SymmetrizeCandidateSet();
