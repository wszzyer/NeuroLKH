...
COORD_MEAN = [4.1662e+06, 8.7300e+05]
COORD_STD = [2.5976e+03, 5.0038e+03]
...
class LaDeDataset(Dataset):
    def __init__(self, file_path, extra_node_feats, edge_feats, problem="tsp"):
        ...
        self.dataset["node_feat"][:, :, :2] = (self.dataset["node_feat"][:, :, :2] - COORD_MEAN) / COORD_STD
        # feature selection
        default_node_dim = get_problem_default_node_feat_dim(problem)
        if extra_node_feats:
            self.dataset["node_feat"][:, :, default_node_dim:] = (
                self.dataset["node_feat"][:, :, default_node_dim:][..., np.concatenate([get_feat_indexes(feat) for feat in extra_node_feats])])
        if edge_feats:
...
class LaDeTestDataset(Dataset):
    def __init__(self, problem, node_feat, edge_feat, edge_index, inverse_edge_index, extra_node_feats_class, edge_feats_class):
        ...
        # coord standarization
        node_feat[:, :, :2] = (node_feat[:, :, :2] - COORD_MEAN) / COORD_STD
        # feature selection
        default_node_dim = get_problem_default_node_feat_dim(problem)
        if extra_node_feats_class:
            node_feat[:, :, default_node_dim:] = (
                node_feat[:, :, default_node_dim:][..., np.concatenate([get_feat_indexes(feat) for feat in extra_node_feats_class])])
        if edge_feats_class:
